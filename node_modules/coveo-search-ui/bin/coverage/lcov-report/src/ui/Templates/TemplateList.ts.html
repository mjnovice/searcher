...
&nbsp;
  instantiateToElement(object: IQueryResult, <span class="missing-if-branch" title="else path not taken" >E</span>instantiateOptions: IInstantiateTemplateOptions = {}): Promise&lt;HTMLElement&gt; {
    let merged = new DefaultInstantiateTemplateOptions().merge(instantiateOptions);
&nbsp;
    const filteredTemplates = _.reject(this.templates, t =&gt; t.role != null);
    for (var i = 0; i &lt; filteredTemplates.length; i++) {
      var promiseOfHTMLElement = filteredTemplates[i].instantiateToElement(object, merged);
      <span class="missing-if-branch" title="else path not taken" >E</span>if (promiseOfHTMLElement != null) {
